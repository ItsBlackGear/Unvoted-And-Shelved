...
import com.cursedcauldron.unvotedandshelved.core.platform.common.BiomeWriter;
import com.cursedcauldron.unvotedandshelved.core.platform.common.BiomeContext;
...
public class BiomeManagerImpl {
    ...
    public static void bootstrap() {
        FMLJavaModLoadingContext.get().getModEventBus().<RegisterEvent>addListener(event -> {
            event.register(ForgeRegistries.Keys.BIOME_MODIFIER_SERIALIZERS, entry -> entry.register(new ResourceLocation(UnvotedAndShelved.MOD_ID, "biome_modifier_codec"), codec = Codec.unit(PlatformBiomeModifier.INSTANCE)));
            event.register(ForgeRegistries.Keys.BIOME_MODIFIERS, entry -> entry.register(new ResourceLocation(UnvotedAndShelved.MOD_ID, "biome_modifier"), PlatformBiomeModifier.INSTANCE));
        });
    }
